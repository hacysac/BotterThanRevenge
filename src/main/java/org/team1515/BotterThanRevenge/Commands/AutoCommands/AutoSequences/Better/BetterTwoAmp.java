...
public class BetterTwoAmp extends SequentialCommandGroup{
    public BetterTwoAmp(Drivetrain drivetrain, Shooter shooter, Indexer indexer, Intake intake, Flip flip, double direction){        
        ...
        double ampX = -direction*Units.inchesToMeters(RobotMap.WALL_TO_AMP - 0.5*RobotMap.CHASSIS_WIDTH); //assuming red
        ...
        //Drive to Amp
        addCommands(new driveLine(drivetrain, firstRotation, pose1, 1));
        ...
        //Score Note 1
        addCommands(new driveLine(drivetrain, -secondRotation, pose1, 1));
        addCommands(new AutoFeed(indexer, RobotMap.AUTO_FEED_TIME));

        //Drive Back
        addCommands(new InstantCommand(()->shooter.end()));
        addCommands(new driveLine(drivetrain, -firstRotation, finalPose, 1));
    }
}
